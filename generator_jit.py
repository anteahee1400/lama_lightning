...
class FFC(nn.Module):
    ...
    def forward(self, x_l, x_g=0):
        ...
        if self.ratio_gout != 1:
            out_xl = self.convl2l.to(x_l.device)(x_l) + self.convg2l.to(x_l.device)(x_g) * g2l_gate
        if self.ratio_gout != 0:
            out_xg = self.convl2g.to(x_l.device)(x_l) * l2g_gate + self.convg2g.to(x_l.device)(x_g)
...
class FFC_BN_ACT(nn.Module):
    ...
    def forward(self, x_l, x_g=0):
        ...
        x_l = self.act_l.to(x_l.device)(self.bn_l.to(x_l.device)(x_l))
        x_g = self.act_g.to(x_l.device)(self.bn_g.to(x_l.device)(x_g))
        return x_l, x_g
...
class ConcatTupleLayer(nn.Module):
    ...
    def forward(self, x_l, x_g):
        ...
        # x_l, x_g = x
        assert isinstance(x_l, torch.Tensor) or isinstance(x_g, torch.Tensor)
...
class FFCResNetGenerator(nn.Module):
    def __init__(
        ...
        assert n_blocks >= 0
        super().__init__()
        # print("init_conv_kwargs", init_conv_kwargs)
        # print("downsample_conv_kwargs", downsample_conv_kwargs)
        # print("resnet_conv_kwargs", resnet_conv_kwargs)
        # print("out_ffc_kwargs", out_ffc_kwargs)
        self.n_downsampling = n_downsampling
        self.n_blocks = n_blocks
        self.downsample_conv_kwargs = downsample_conv_kwargs
        self.resnet_conv_kwargs = resnet_conv_kwargs
        ...
        init_model = [
            ...
            FFC_BN_ACT(
                ...
                norm_layer=norm_layer,
                activation_layer=activation_layer,
                ratio_gin=init_conv_kwargs["ratio_gin"],
                ratio_gout=init_conv_kwargs["ratio_gout"],
                enable_lfu=init_conv_kwargs["enable_lfu"],
            ),
        ]
        self.init_model = nn.Sequential(*init_model)

        ### downsample
        self.downsample_model = lambda mult, kw: FFC_BN_ACT(
            ...
            norm_layer=norm_layer,
            activation_layer=activation_layer,
            ratio_gin=kw["ratio_gin"],
            ratio_gout=kw["ratio_gout"],
            enable_lfu=kw["enable_lfu"],
        )
        ...
        #             norm_layer=norm_layer,
        #             activation_layer=activation_layer,
        #             ratio_gin=cur_conv_kwargs["ratio_gin"],
        #             ratio_gout=cur_conv_kwargs["ratio_gout"],
        #             enable_lfu=cur_conv_kwargs["enable_lfu"],
        #         )
        #     ]
        ...
        ### resnet blocks
        self.resnet_block = FFCResnetBlock(
            feats_num_bottleneck,
            padding_type=padding_type,
            activation_layer=activation_layer,
            norm_layer=norm_layer,
            ratio_gin=resnet_conv_kwargs["ratio_gin"],
            ratio_gout=resnet_conv_kwargs["ratio_gout"],
            enable_lfu=resnet_conv_kwargs["enable_lfu"],
        )
        ...
        if add_out_act:
            final_model.append(get_activation("tanh" if add_out_act is True else add_out_act))
        self.final_model = nn.Sequential(*final_model)

    
    def forward(self, input):
        x_l, x_g = self.init_model(input)
        for i in range(self.n_downsampling):
            mult = 2 ** i
            if i == self.n_downsampling - 1:
                cur_conv_kwargs = dict(self.downsample_conv_kwargs)
                cur_conv_kwargs["ratio_gout"] = self.resnet_conv_kwargs.get("ratio_gin", 0)
            else:
                cur_conv_kwargs = self.downsample_conv_kwargs
            x_l, x_g = self.downsample_model(mult, cur_conv_kwargs)(x_l, x_g)
        for i in range(self.n_blocks):
            x_l, x_g = self.resnet_block(x_l, x_g)
        out = self.concat_layer(x_l, x_g)
        out = self.upsample_model(out)
...
def make_generator(config):
    ...
    #     return MultiDilatedGlobalGenerator(**config)

    # if kind == 'pix2pixhd_global':
    #     return GlobalGenerator(**config)

    if kind == "ffc_resnet":
        return FFCResNetGenerator(**config)
